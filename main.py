...
from utils.completer import navii_completer
...
def main():
    shell_clear(None)  # NOTE: argument is required yet not used
 ...
 |__|__||__|__| \ _ /  |____||____|
    ...
    if USING_READLINE and readline:
        try:
            ...
            readline.set_completer(navii_completer)
    ...
    while True:
        try:
            ...
            command_list = [cmd.strip() for cmd in user_input.split(";") if cmd.strip()]

            for command_to_execute in command_list:
                expanded = expand_vars_and_tilde(command_to_execute)
